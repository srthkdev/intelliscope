...
// POST handler to pause an investigation
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // 2. Update the investigation status to 'paused'
    // 3. Save the updated investigation to the database
    ...
    return NextResponse.json(
      { message: 'Failed to pause investigation', error: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}